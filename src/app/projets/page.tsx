...
function ProjetsInner() {
    ...
    if (modal === 'edit' && selected) {
      ...
      if (newP) {
        ...
        await supabase.from('taches').insert([{
          ...
          subtasks: [
            { label: 'Envoyer le devis', done: false },
            { label: 'Confirmer la date de tournage', done: false },
            { label: 'Préparer le matériel', done: false },
          ],
          done: false,
        }])
        ...
        if (data.date_deadline) {
          ...
          await supabase.from('evenements').insert([{
            ...
            lieu:       '',
            contact:    data.client_nom || '',
            materiel:   '',
            notes:      `Deadline du projet "${data.nom}"`,
            checks:     [],
          }])
        }
        ...
        await supabase.from('devis').insert([{
          ...
          date_echeance:  data.date_deadline || '',
          montant:        data.budget || 0,
          statut:         'draft',
          lignes:         [{ desc: data.nom, soustitems: [], qte: 1, pu: data.budget || 0 }],
        }])
      }
    }
  }
  ...
  async function onDrop(e: React.DragEvent, colId: string) {
    e.preventDefault()
    if (!dragId) return
    await moveProject(dragId, colId)
    setDragId(null)
  }
      ...
      <div style={{ display: 'flex', flexDirection: 'column', height: '100vh', overflow: 'hidden' }}>
        ...
        <div style={{ padding: '1rem 1.25rem', borderBottom: '0.5px solid var(--border)', display: 'flex', alignItems: 'center', gap: '10px', flexShrink: 0, background: 'var(--card-bg)' }}>
          <div style={{ fontSize: '16px', fontWeight: 500, color: 'var(--text-primary)', flex: 1 }}>
            Projets <span style={{ fontSize: '13px', color: 'var(--text-muted)', fontWeight: 400 }}>({projects.length})</span>
          </div>
        ...
        <div style={{ flex: 1, display: 'flex', overflow: 'hidden' }}>
          <div style={{ flex: 1, overflow: 'auto', padding: '1rem', background: 'var(--bg-primary)' }}>
            {view === 'kanban' ? (
                    ...
                    <div key={col.id} onDragOver={onDragOver} onDrop={e => onDrop(e, col.id)}
                      ...
                      <div style={{ flex: 1, overflowY: 'auto', padding: '.5rem' }}>
                        {cards.map(p => {
                          ...
                          return (
                            <div key={p.id} draggable onDragStart={() => onDragStart(p.id)} onClick={() => setSelectedId(p.id)}
                        ...
                        {cards.length === 0 && (
                          <div style={{ fontSize: '11px', color: 'var(--text-faint)', textAlign: 'center', padding: '1rem 0' }}>Déposez ici</div>
                        )}
                      </div>
                    </div>
                  )
              ...
              <table style={{ width: '100%', borderCollapse: 'collapse' }}>
                <thead>
                  <tr>
                    {['Projet','Client','Type','Budget','Deadline','Priorité','Avancement'].map(h => (
                ...
                <tbody>
                  {projects.map(p => {
                    const tp = TYPES[p.type] || TYPES.corporate
                    return (
                      <tr key={p.id} onClick={() => { setSelectedId(p.id); setView('kanban') }} style={{ cursor: 'pointer' }}>
                        ...
                        <td style={{ padding: '9px 10px', borderBottom: '0.5px solid var(--border)' }}>
                          <span style={{ fontSize: '10px', padding: '2px 7px', borderRadius: '7px', background: tp.bg, color: tp.color }}>{tp.label}</span>
                        </td>
          ...
          {selected && (
            <div style={{ width: '280px', borderLeft: '0.5px solid var(--border)', background: 'var(--bg-tertiary)', display: 'flex', flexDirection: 'column', flexShrink: 0 }}>
              <div style={{ padding: '1rem', borderBottom: '0.5px solid var(--border-subtle)', flexShrink: 0 }}>
                <span style={{ fontSize: '10px', padding: '2px 8px', borderRadius: '8px', background: (TYPES[selected.type] || TYPES.corporate).bg, color: (TYPES[selected.type] || TYPES.corporate).color, display: 'inline-block', marginBottom: '.5rem' }}>
                  {(TYPES[selected.type] || TYPES.corporate).label}
                </span>
              ...
              <div style={{ flex: 1, overflowY: 'auto', padding: '1rem' }}>
                ...
                <div style={{ fontSize: '11px', color: 'var(--text-faint)', textTransform: 'uppercase', letterSpacing: '.5px', fontWeight: 500, marginBottom: '.5rem' }}>
                  Checklist — {doneSubs}/12
                </div>
                <div style={{ display: 'flex', flexDirection: 'column', gap: '4px', marginBottom: '1rem' }}>
                  {CHECKLIST.map((label, idx) => (
                    <div key={idx} onClick={() => handleToggleCheck(idx)} style={{ display: 'flex', alignItems: 'center', gap: '8px', padding: '4px 0', cursor: 'pointer' }}>
                      <div style={{ width: '14px', height: '14px', borderRadius: '3px', flexShrink: 0, border: '0.5px solid', borderColor: selected.checks[idx] ? '#ff6b2b' : 'var(--border-subtle)', background: selected.checks[idx] ? '#ff6b2b' : 'transparent', display: 'flex', alignItems: 'center', justifyContent: 'center', fontSize: '9px', color: '#fff' }}>
                        {selected.checks[idx] ? '✓' : ''}
                      </div>
